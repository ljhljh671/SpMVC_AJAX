...
$(function(){
	...
	$(".p-tr").click(function(){
		// id값을 추출할 수 있으면 몇번째  tr이 클릭 되었는지
		// 알수가 있다.
		...
		// attr() 함수
		// $(...) : 선택자, selector 라고 하는 객체의
		// 속성값을 getter하거나 setter 하는 함수다
...
<section>
	<article id = "p-list">
			<table class="w3-table-all" id = "ptable">
			<tr>
				<th>NO</th>
				<th>상품코드</th>
				<th>상품이름</th>
		...
		<c:choose>
			<c:when test="${empty PLIST}">
				<tr><td colspan="10">상품리스트가 없습니다.</td></tr>
			</c:when>
			<c:otherwise>
				<c:forEach var="PVO" items="${PLIST}" varStatus="i">
					<tr class="p-tr" 
						...
						id="${i.index}">
							...
							<td>${PVO.p_ccode}</td>
							<td>${PVO.p_cname}</td>
							<td><c:if test="${PVO.p_vat=='1'}">과세</c:if>
